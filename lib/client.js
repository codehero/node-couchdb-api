...
proto._request = function (options, callback) {
				...
				res.on("data", function (chunk) {
					...
					if (options.changes) {
						...
						if (chunk) {
							...
							else if("last_seq" in chunk){
								logger.info("Last seq received, reconnecting.", chunk);
								res.emit("last_seq", chunk.last_seq);
							}
						}
					}
				});
	...
	if (options.body) {
		...
		else if ("content-type" in req._headers) {
    ...
    debug: {
        ...
        set: function (v) {
            	...
            	this._debug = false;
						...
						if(!(v in consoleMap))
							throw new Error("Undefined debug level: " + v);
            this._debug = consoleMap[v];
            this._debugType = v;
        }
    }
});
